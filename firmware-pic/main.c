...
unsigned char ReceivedDataBuffer[6];
unsigned char ToSendDataBuffer[6];
#pragma udata

USB_HANDLE USBOutHandle = 0;
USB_HANDLE USBInHandle = 0;
BOOL keptPressed; //utile per mandare 1 solo interrupt a pressione del tasto
...
void i2c_init(void) {
  ...
  // Initial the PIC18F14K50 MSSP Peripheral I2C Master Mode
  // I2C Master Clock Speed: 100Khz=> 48000000 / ((4 * (SSPADD + 1)) = 48000000 / (4 * (119 + 1))
  ...
  SSPADD = 119;         // Standard I2C Clock speed: 100 kHz  
...
unsigned char Read_Temp(unsigned char *dval) {
  ...
  // Get the High Byte of I2C Temp Sensor
  hidata=i2c_read();   

  // Send Acknowledge to the Slave
  //i2c_master_ack(I2C_DATA_ACK);     

  // Get the Low Byte of I2C Temp Sensor
  *dval=i2c_read();      
...
void ProcessIO(void)
{   
	unsigned char dvalue=0x00,temp_value=0x00,temp,sign;
	int i;
    ...
    {   
        ...
        {
            case 0x80:  //Toggle LEDs command			//Stop blinking the LEDs automatically, going to manually control them now.
                mLED_1_Toggle();
                break;
            case 0x81:  //Get push button state
				//resetto il buffer
				for(i=0;i<6;i++){ToSendDataBuffer[i]=0;}
				...
				if(action_sw == 1)							//pushbutton not pressed, pull up resistor on circuit board is pulling the PORT pin high
				{
                ...
                {
                    USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&ToSendDataBuffer[0],6);
                }
                break;
			...
			case 0x50:
				//resetto il buffer
				for(i=0;i<6;i++){ToSendDataBuffer[i]=0;}
				temp_value=Read_Temp(&dvalue);
		      	//temp=temp_value & 0x7F;
		      	//sign=temp_value & 0x80;
				
				ToSendDataBuffer[0] = 0x50;  	
				ToSendDataBuffer[1] = temp_value;  	
				ToSendDataBuffer[2] = dvalue;
				if(!HIDTxHandleBusy(USBInHandle)){	
					USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&ToSendDataBuffer[0],6);
                }
        ...
        USBOutHandle = HIDRxPacket(HID_EP,(BYTE*)&ReceivedDataBuffer,6);
    }
